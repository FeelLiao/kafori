...
def test_init_global_logger_file_only(tmp_path):
    ...
    init_global_logger(log_file, enable_console=False,
                       enable_file=True, level=logging.INFO)
...
def test_init_global_logger_console_only():
    log_file = Path("dummy.log")
    init_global_logger(log_file, enable_console=True,
                       enable_file=False, level=logging.WARNING)
    logger = logging.getLogger()
    handlers = logger.handlers
    assert len(handlers) == 1
...
def test_init_global_logger_both_handlers(tmp_path):
    ...
    init_global_logger(log_file, enable_console=True,
                       enable_file=True, level=logging.ERROR)
    logger = logging.getLogger()
